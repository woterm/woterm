...
initialize_options(Options * options)
{
    ...
    options->remote_command = NULL;// xstrdup("which rza > /dev/null 2>&1 || which sza > /dev/null 2>&1 || echo \"Please install lrzsz program for upload or download.\"");
...
fill_default_options(Options * options)
{
	...
	if (options->number_of_password_prompts == -1)
		options->number_of_password_prompts = 4;
